...
describe('ConfigCacheService', () => {
  ...
  describe('clear operations', () => {
    it('should clear specific key', () => {
      ...
      const key2 = 'FRONTEND_URL' as keyof ConfigVariables;

      service.set(key1, true);
      service.set(key2, 'http://0.0.0.0:3000/');
      service.clear(key1);

      expect(service.get(key1)).toBeUndefined();
      expect(service.get(key2)).toBe('http://0.0.0.0:3000/');
    });
  ...
  describe('getCacheInfo', () => {
    it('should return correct cache information', () => {
      ...
      service.set(key1, true);
      service.set(key2, 'http://localhost:3000/');
